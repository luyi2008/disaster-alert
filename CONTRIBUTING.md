...
```bash
cp .env.example .env
# 在 .env 中填写 ALERT_DETAIL_BASE_URL、ALERT_SIGNING_KEY、BFF_SERVICE_TOKEN 等
cargo run
```
...
Vite 会把 `/api` 和 `/health` 代理到本机 API。浏览器打开 Vite 提示的本地地址，不要只开 `http://127.0.0.1:30010/` 当网页。注意本仓库的接口路径已统一加上 `/api/` 前缀（见 [README.md](README.md) 的接口列表），`disaster-alert-web` 里调用这些接口的地方需要同步更新。
...
- `POST /api/simulate` 与 `GET /api/history` 是旁路测试口：simulate 验 BFF 服务凭证，设备来自 `device_ID_list`；history 仍可用 Bark Bearer 标注距离。只对列表中的已存订阅调用 Bark，**不**进入 `EventRuntime` / inbox / 匹配 / 投递账本，也**不**对全站扇出。真实 Bark Key 不得写入测试、文档或提交。架构与测试方法见 [docs/simulate.md](docs/simulate.md)
- `GET /api/subscriptions` 用 `Authorization: Bearer` Bark Key 返回该 Key 当前激活的地点和规则。不得接受空 Key，也不得列出其他设备。已取消、仅待确认或不存在的订阅返回 HTTP 200 且 `success` 为 false，不返回 4xx
...
- 另有未写入 README / OpenAPI 的运营只读接口：列出当前激活订阅的 Bark Key、按 Key 返回地点和规则（`GET /api/admin/subscriptions`，空结果同样 HTTP 200 且 `success` 为 false，不返回 4xx）、查询投递账本（`device_key` 可空，空则返回全部成功投递，每条含 Bark Key 与事件内容）。当前无鉴权，后续补上；不要把这些路径写进用户文档
- `GET /api/events` 列出保留期内已接入灾害的事件详情（含地震），支持 `limit`（默认 50，最大 200）与 `cursor` 游标翻页，供前端滚动加载；响应 `next_cursor` 为空表示已到末页。当前同样无鉴权，后续补上；不要把这个路径写进用户文档
- 退订接口只返回操作结果，不回显订阅内容
