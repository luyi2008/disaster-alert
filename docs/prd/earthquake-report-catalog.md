...
公开订阅、Bark 推送、匹配规则不在本文范围。查询口仍是未写入用户文档的 `GET /api/events`。
...
- 不把地震预警的未命中记录做成目录（EEW 连报会把运营列表撑满）。
- 不把演练报写入目录（`IGNORE_TRAINING=true` 时训练不是真实测定）。
- 不扩展 ingest。Wolfx `cenc_eqlist` 仍只取 No1。目录等于「本进程运行期间见过的速报」，不是台网全量历史。
...
- 无订阅时接入一条 CENC 速报：`GET /api/events` 能看到该 incident，`has_matched_subscribers` 为 false，Bark 未发送。
